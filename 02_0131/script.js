...
const shine_radius = 100; // 照射範囲の半径
...
/* ---- テスト用 ---- */
function createSquareWithCircularHole(elementId, squareSize, holeRadius) {
  const element = document.getElementById(elementId);
  const points = createCirclePoints(squareSize / 2, squareSize / 2, holeRadius, 50); // 50点で円を近似
...
function createCirclePoints(cx, cy, radius, numPoints) {
  const points = [];
  for (let i = 0; i < numPoints; i++) {
      const theta = (i / numPoints) * 2 * Math.PI;
      const x = cx + radius * Math.cos(theta);
      const y = cy + radius * Math.sin(theta);
      points.push({ x: (x / 200) * 100, y: (y / 200) * 100 });
  }
  return points;
}
...
function toggleGameStatus() {
  ...
  if (!isGameActive) {
    ...
    bullet.style.left = `${gameAreaRect.width / 2 - 10}px`; // 横の中心
    bullet.style.top = `${gameAreaRect.height / 2 - 40}px`; // 縦の中心
...
function onGameAreaClick(event) {
  /* ---- オーバーレイのクリップ ---- */
    // オーバーレイ内でのクリック位置を取得
    const overlayX = event.clientX - layAreaRect.left;
    const overlayY = event.clientY - layAreaRect.top;
    const radius = 100; // クリッピングする円の半径
    const points = [];

    for (let i = 7; i >= 0; i--){
      const angle = Math.PI / 4 * i;

      const x = (overlayX + radius * Math.cos(angle)) / layAreaRect.width * 100;
      const y = (overlayY + radius * Math.sin(angle)) / layAreaRect.height * 100;
      points.push(`${x}% ${y}%`);
    }

    const height = `${overlayY - radius}px`;
    console.log(height);
    //layArea.style.clipPath = `polygon(0% 0%, 100% 0%, 100% 100%, 0% 100%, 0% 0%, ${points.join(', ')})`;
    //layArea.style.clipPath = `polygon(0% 0%, 100% 0%, 100% 100%, 0% 100%, 0% ` + `${height}, ` + `${points.join(', ')})`;
    layArea.style.clipPath = `polygon(${points.join(', ')})`;
